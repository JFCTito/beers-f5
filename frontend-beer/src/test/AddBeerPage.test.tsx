...
import { AddBeerPage } from "../pages/AllBeersPage/AddBeerPage";
...
describe("Iteration 7", () => {
  describe("AddBeerPage component", () => {
    ...
    test("renders the 'description' input field", async () => {
      const descriptionInput = screen.getByRole("textarea", {
        name: "description",
      });
      await waitFor(() => {
    ...
    test("renders the 'first_brewed' input field", async () => {
      const firstBrewedInput = screen.getByRole("input", {
        name: "first_brewed",
      });
      await waitFor(() => {
    ...
    test("renders the 'brewers_tips' input field", async () => {
      const brewersTipsInput = screen.getByRole("input", {
        name: "brewers_tips",
      });
      await waitFor(() => {
    ...
    test("renders the 'contributed_by' input field", async () => {
      const contributedByInput = screen.getByRole("input", {
        name: "contributed_by",
      });
      await waitFor(() => {
    ...
    test("renders the 'attenuation_level' input field", async () => {
      const attenuationInput = screen.getByRole("input", {
